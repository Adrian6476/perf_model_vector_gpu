...
int main(int argc, char** argv) {
    ...
    // Create CUDA streams for task parallelism
    cudaStream_t stream1, stream2;
    ErrorCheck(cudaStreamCreate(&stream1), __FILE__, __LINE__);
    ErrorCheck(cudaStreamCreate(&stream2), __FILE__, __LINE__);

    // Launch kernels using streams for task parallelism
    function_d_kernel<<<1, blockSize, 0, stream1>>>(d_u, d_v, d_s, N);
    function_b_kernel<<<gridSize, blockSize, 0, stream1>>>(2, d_u, d_v, d_x, N);

    function_c_kernel<<<gridSize, blockSize, 0, stream2>>>(0, d_x, d_y, d_z, N);
    function_a_kernel<<<gridSize, blockSize, 0, stream2>>>(d_A, d_x, d_y, N);

    // Synchronize streams
    ErrorCheck(cudaStreamSynchronize(stream1), __FILE__, __LINE__);
    ErrorCheck(cudaStreamSynchronize(stream2), __FILE__, __LINE__);
    ...
    ErrorCheck(cudaStreamDestroy(stream1), __FILE__, __LINE__);
    ErrorCheck(cudaStreamDestroy(stream2), __FILE__, __LINE__);
